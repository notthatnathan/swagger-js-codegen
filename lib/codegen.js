            ...
            _.forEach(op.responses, function(val, key) {
                if (key.startsWith('2')) {
                    if (val.schema) {
                        ...
                        // add top level response keys
                        method.responseProperties = Object.keys(val.schema.properties || {}).map(prop => {
                            return {
                                name: prop,
                                type: val.schema.properties[prop].type || '',
                                description: val.schema.properties[prop].description || '',
                            }
                        });
